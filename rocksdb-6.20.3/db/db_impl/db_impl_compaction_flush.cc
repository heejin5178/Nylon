...
Status DBImpl::FlushMemTableToOutputFile(
  ...
  if (s.ok()) {
    // hj debug point
    /*
    while (flush_job.GetMemTables().size() <= 2) {
  ...
  if (s.ok()) {
    ...
    /*
    ROCKS_LOG_BUFFER(log_buffer,
                       "[%s] HJ LOG - dynamic max flushes : %", PRId64 ,"\n",
                       cfd->GetName(), cfd->dynamic_max_flushes);
		       */
    need_cancel = false;
  }
...
Status DBImpl::AtomicFlushMemTablesToOutputFiles(
  ...
  if (s.ok()) {
    for (int i = 0; i != num_cfs; ++i) {
      // hj debug point
      /*
      while (jobs[i]->GetMemTables().size() <= 2) {
	  std::this_thread::yield();
    	  // hj : do nothing
      }
      */
    ...
    for (const auto& newf : c->edit()->GetNewFiles()) {
      (*output_file_names)
          .push_back(TableFileName(c->immutable_cf_options()->cf_paths,
                                   ...
                                   newf.second.fd.GetPathId())); // hj debug point
    }
  }
...
void DBImpl::MaybeScheduleFlushOrCompaction(int is_high) {
  LogBuffer log_buffer(InfoLogLevel::INFO_LEVEL,
                       immutable_db_options_.info_log.get());
  /*
	if (is_high == 1) {
		fprintf(stdout, "is high is %d, %d, %d\n", is_high, bg_compaction_scheduled_, GetBGJobLimits().max_compactions);
	}	
	else {
		fprintf(stdout, "is high is not 1,  %d\n", is_high);
	}
  */
  mutex_.AssertHeld();
  ...
  // hj debug point
  // bool is_large_flush = GetCurMemListSZ() > 2;
  ...
  while (bg_compaction_scheduled_ < bg_job_limits.max_compactions &&
    ...
    if (is_high == 1) {
	// fprintf(stdout, "High compaction occured!\n");
	ca->compaction_pri_ = Env::Priority::HIGH;
    	env_->Schedule(&DBImpl::BGWorkMidCompaction, ca, Env::Priority::HIGH, this,
                   &DBImpl::UnscheduleCompactionCallback);
    }
    else
    	env_->Schedule(&DBImpl::BGWorkCompaction, ca, Env::Priority::LOW, this,
                   &DBImpl::UnscheduleCompactionCallback);
    /*
    if (ca->is_high_compaction)
	    env_->Schedule(&DBImpl::BGWorkCompaction, ca, Env::Priority::MID, this,
			   &DBImpl::UnscheduleCompactionCallback);
			   */
  }
}
...
void DBImpl::SchedulePendingFlush(const FlushRequest& flush_req,
  ...
  if (!immutable_db_options_.atomic_flush) {
    ...
    // hj added
    // bool is_mem_list_full = cfd->imm().GetCurMemListSZ() > 3;
    assert(cfd);
...
void DBImpl::BGWorkCompaction(void* arg) {
  ...
  static_cast_with_check<DBImpl>(ca.db)->BackgroundCallCompaction(
      prepicked_compaction, Env::Priority::LOW, s_);
  delete prepicked_compaction;
  if (s_.IsHighCompaction())
	  ca.is_high_compaction = true;
}

/*
Status DBImpl::BGWorkCompaction(void* arg) {
  CompactionArg ca = *(reinterpret_cast<CompactionArg*>(arg));
  delete reinterpret_cast<CompactionArg*>(arg);
  IOSTATS_SET_THREAD_POOL_ID(Env::Priority::LOW);
  TEST_SYNC_POINT("DBImpl::BGWorkCompaction");
  ...
  Status s;
  static_cast_with_check<DBImpl>(ca.db)->BackgroundCallCompaction(
      prepicked_compaction, Env::Priority::LOW, s);
  delete prepicked_compaction;
  return s;
}
*/
...
void DBImpl::BGWorkBottomCompaction(void* arg) {
  ...
  Status s_ = Status::OK();
  ca.db->BackgroundCallCompaction(prepicked_compaction, Env::Priority::BOTTOM, s_);
  delete prepicked_compaction;
}

void DBImpl::BGWorkMidCompaction(void* arg) {
  CompactionArg ca = *(static_cast<CompactionArg*>(arg));
  delete static_cast<CompactionArg*>(arg);
  IOSTATS_SET_THREAD_POOL_ID(Env::Priority::HIGH);
  TEST_SYNC_POINT("DBImpl::BGWorkMidCompaction");
  auto* prepicked_compaction = ca.prepicked_compaction;
  /*
  assert(prepicked_compaction && prepicked_compaction->compaction &&
         !prepicked_compaction->manual_compaction_state);
	 */
  Status s_ = Status::OK();
  ca.db->BackgroundCallCompaction(prepicked_compaction, Env::Priority::HIGH, s_, true);
  delete prepicked_compaction;
}
...
void DBImpl::UnscheduleCompactionCallback(void* arg) {
  ...
  } else if (Env::Priority::MID == compaction_pri) {
    // Decrement bg_compaction_scheduled_ if priority is LOW
    ca_ptr->db->bg_mid_compaction_scheduled_--;
  ...
  *reason = FlushReason::kOthers;
  ...
  while (!flush_queue_.empty()) {
    ...
    for (const auto& iter : flush_req) {
      ...
      else if(cfd->imm()->GetCurMemListSZ() <= cfd->dynamic_max_flushes)  { // main hj: for multiple flushes
	      // todo : 2 will consider BW
	  ROCKS_LOG_BUFFER(
          log_buffer,
          "Small flush will be not flushed");
        column_families_not_to_flush.push_back(cfd);
        continue;
      }
  ...
  if (!bg_flush_args.empty()) {
    ...
    // hj : update status for performance of flush 
    ColumnFamilyData* flushed_cfd = bg_flush_args[0].cfd_;
    bool is_first_flush = flushed_cfd->max_flush_nanos == 0;
    bool update_flush_nanos = false;
    int64_t prev_max_flush_nanos = flushed_cfd->max_flush_nanos;
    int64_t prev_flush, after_flush = 0;
    prev_flush = IOSTATS(write_nanos);
    status = FlushMemTablesToOutputFiles(bg_flush_args, made_progress,
                                         job_context, log_buffer, thread_pri);
    after_flush = IOSTATS(write_nanos);
    if (flushed_cfd->max_flush_nanos <= (after_flush - prev_flush)) {
	    update_flush_nanos = true;
	    flushed_cfd->max_flush_nanos = after_flush - prev_flush;
    }

    double gap_threshold = 0.001;
    if (!is_first_flush && update_flush_nanos) { // dynamic_max_flushes will be set 
	    int64_t diff_flush_nanos = flushed_cfd->max_flush_nanos - prev_max_flush_nanos;
	    if (diff_flush_nanos > 0) {
		if (diff_flush_nanos >= prev_max_flush_nanos * gap_threshold) {
		 	flushed_cfd->dynamic_max_flushes--; 
		      	if (flushed_cfd->dynamic_max_flushes <= 2)
				flushed_cfd->dynamic_max_flushes = 2;	
		}
	    }
	    else {
		if (std::abs(diff_flush_nanos) >= prev_max_flush_nanos * gap_threshold) {
		 	flushed_cfd->dynamic_max_flushes++;  
		      	if (flushed_cfd->dynamic_max_flushes >= 9)
				flushed_cfd->dynamic_max_flushes = 9;	 
		}
	    }
...
void DBImpl::BackgroundCallCompaction(PrepickedCompaction* prepicked_compaction,
                                      Env::Priority bg_thread_pri, Status& s_, bool high_compaction) {
  bool made_progress = false;
  JobContext job_context(next_job_id_.fetch_add(1), true);
                       ...
                       immutable_db_options_.info_log.get());
  {
	  ...
	  // hj debug
	  /*
	  if (Env::Priority::MID == bg_thread_pri) {
		fprintf(stderr, "BG compaction with MID");
		exit(1);
	  }
	  */
    InstrumentedMutexLock l(&mutex_);
    ...
    assert((bg_thread_pri == Env::Priority::BOTTOM &&
           ...
           (bg_thread_pri == Env::Priority::LOW && bg_compaction_scheduled_) || bg_thread_pri == Env::Priority::HIGH);
    Status s = BackgroundCompaction(&made_progress, &job_context, &log_buffer,
                                    prepicked_compaction, bg_thread_pri, high_compaction);
    if (s.IsHighCompaction()) {
	    
      	   ROCKS_LOG_BUFFER(&log_buffer, "Obsolete");
	   /*
	    s = BackgroundCompaction(&made_progress, &job_context, &log_buffer,
					    prepicked_compaction, Env::Priority::MID);
					    */
    ...
    num_running_compactions_--;
    if (bg_thread_pri == Env::Priority::LOW || bg_thread_pri == Env::Priority::HIGH) {
      bg_compaction_scheduled_--;
    } else {
      assert(bg_thread_pri == Env::Priority::BOTTOM);
      bg_bottom_compaction_scheduled_--;
...
// hj debug point
// it will seperate L0-L1 compaction and others
Status DBImpl::BackgroundCompaction(bool* made_progress,
                                    JobContext* job_context,
                                    LogBuffer* log_buffer,
                                    PrepickedCompaction* prepicked_compaction,
                                    Env::Priority thread_pri, 
				    bool high_compaction) {
	 
      // ROCKS_LOG_BUFFER(log_buffer, "Priority: %s", env_->PriorityToString(thread_pri)); 	
  ...
  *made_progress = false;
    ...
    if (!mutable_cf_options->disable_auto_compactions && !cfd->IsDropped()) {
      ...
      if (c != nullptr) {
        ...
        if (!enough_room) {
          ...
          // will sleep if !s.ok()
          status = Status::CompactionTooLarge();
        } else if (c->output_level() == 1 && c->level() == 0 && !high_compaction) {
    	  ROCKS_LOG_BUFFER(log_buffer, "Middle compaction return");
          AddToCompactionQueue(cfd);
          ++unscheduled_compactions_;
	  MaybeScheduleFlushOrCompaction(1);
	} else {
          // update statistics
  ...
  /*
  if (!c && c->output_level() == 1 && c->level() == 0 && !high_compaction) {
	  ROCKS_LOG_BUFFER(log_buffer, "Middle compaction return");
	  AddToCompactionQueue(cfd);
	  ++unscheduled_compactions_;
	  MaybeScheduleFlushOrCompaction(1);
  ...
  */
  if (!c) {
    ...
    *made_progress = true;
    ...
    for (unsigned int l = 0; l < c->num_input_levels(); l++) {
      ...
      if (c->level(l) == 0 && c->output_level() == 1) {
	    if (thread_pri == Env::Priority::MID) {
		ROCKS_LOG_BUFFER(log_buffer, "Middle compaction returned2");
	    }
      }
    }
    ...
    *made_progress = true;
    ...
    TEST_SYNC_POINT_CALLBACK("DBImpl::BackgroundCompaction:AfterCompaction",
                             c->column_family_data());
  } else if (!is_prepicked && c->output_level() > 1 &&
             (c->output_level() ==
                 c->column_family_data()
                     ->current()
                     ->storage_info()
                     ->MaxOutputLevel(
                         immutable_db_options_.allow_ingest_behind) || c->output_level() >= lower_level_) ) {
             // env_->GetBackgroundThreads(Env::Priority::BOTTOM) > 0) {
    // Forward compactions involving last level to the bottom pool if it exists,
    // such that compactions unlikely to contribute to write stalls can be
    // delayed or deprioritized.
    TEST_SYNC_POINT("DBImpl::BackgroundCompaction:ForwardToBottomPriPool");
    ROCKS_LOG_BUFFER(log_buffer, "Bottom compaction occured\n");
    CompactionArg* ca = new CompactionArg;
    ca->db = this;
    ...
    if (thread_pri == Env::Priority::MID) {
    	ROCKS_LOG_BUFFER(log_buffer, "Middle compaction returned");
    }
    ...
    lower_level_ = compaction_job.lower_level + 1; // hj: it prohibits mixed level
  ...
  if (c != nullptr) {
    ...
    *made_progress = true;
    ...
    if (is_mid) 
	    ROCKS_LOG_BUFFER(log_buffer, "Middle compaction also nofityoncompactionbegin..");
