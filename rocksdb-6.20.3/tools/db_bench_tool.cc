...
DEFINE_string(client_tail_log, "", "[HJ Custom option] client tali log path.");

// hj added

DEFINE_string(upper_db, "", "[HJ Custom option] Use the upper db with the following name.");
DEFINE_string(lower_db, "", "[HJ Custom option] Use the lower db with the following name.");

DEFINE_int64(upper_db_sz, 0, 
             "Target size of upper db");
DEFINE_int64(lower_db_sz, 0,
             "Target size of lower db");
...
// Current the following isn't equivalent to OS_LINUX.
 ...
 private:
  ...
  void InitializeOptionsFromFlags(Options* opts) {
    ...
    // hj added
    if (FLAGS_upper_db != "" && FLAGS_lower_db != "") {
	    std::cout << "FLAGS upper db sz: " << FLAGS_upper_db_sz << ", lower db sz: " << FLAGS_lower_db_sz << std::endl;
	    options.db_paths.emplace_back(FLAGS_upper_db, FLAGS_upper_db_sz);
	    options.db_paths.emplace_back(FLAGS_lower_db, FLAGS_lower_db_sz);
    }
  ...
  void DoWrite(ThreadState* thread, WriteMode write_mode) {
    ...
    while (!duration.Done(entries_per_batch_)) {
      ...
      std::fstream data;
      data.open(FLAGS_client_tail_log, std::ios::app);
      for (int64_t j = 0; j < entries_per_batch_; j++) {
	auto client_start_time = std::chrono::high_resolution_clock::now();
	...
	auto client_end_time = std::chrono::high_resolution_clock::now();
	double client_time =
		std::chrono::duration_cast<std::chrono::nanoseconds>(client_end_time -
								     client_start_time)
		    .count();
	//std::time_t start_time = std::chrono::system_clock::to_time_t(client_start_time);
	//std::time_t end_time = std::chrono::system_clock::to_time_t(client_end_time);

	data << client_start_time.time_since_epoch().count() << ", " << client_end_time.time_since_epoch().count() << ", " << client_time << std::endl;
	data.close();
      }
